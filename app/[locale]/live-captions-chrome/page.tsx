                ...
                <thead>
                  <tr className="bg-gray-100 border-b-2 border-gray-300">
                    {comparisonRows[0].map((header: string, idx: number) => (
                      <th key={idx} className="px-6 py-4 text-left font-bold text-gray-900 border-r border-gray-200 last:border-r-0">
                        {header}
                      </th>
                    ))}
                  </tr>
                    ...
                    <tr key={rowIdx} className={rowIdx % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                      {row.map((cell: string, cellIdx: number) => (
                        <td
                          ...
                          } ${cellIdx === row.length - 1 && cell === 'Yes — click anywhere to edit' ? 'bg-green-50' : ''}`}
                        >
                          {cell}
                        </td>
                      ))}
